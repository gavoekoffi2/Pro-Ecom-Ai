...
const STATUS_OPTIONS: { label: string; value: ConversationStatus; color: string }[] = [
  { label: "Ouverte", value: "open", color: "text-primary" },
  { label: "En attente", value: "pending", color: "text-amber-400" },
  { label: "Fermée", value: "closed", color: "text-slate-400" },
];
...
export function MessageThread({
  ...
  const sessionInfo = useMemo(() => {
    ...
    if (!lastCustomerMsg) return { expired: true, remaining: "Aucun message du client" };

    const hoursSince = differenceInHours(new Date(), new Date(lastCustomerMsg.created_at));
    const expired = hoursSince >= 24;

    if (expired) {
      return { expired: true, remaining: "Expirée" };
    }
    ...
    async (template: MessageTemplate, params: string[]) => {
      ...
      try {
        ...
        if (!res.ok) {
          ...
          toast.error(`Échec de l'envoi du modèle : ${reason}`);
          onUpdateMessage(tempId, { status: "failed" });
          return;
        }
  ...
  const contactDisplayName = contact?.name || contact?.phone || "Client";
  ...
  const authorLabelFor = useCallback(
    (m: Message): string => {
      ...
      return isAgentMsg ? "Vous" : contactDisplayName;
    },
    [contactDisplayName],
  );
  ...
  const postReaction = useCallback(
    async (messageId: string, emoji: string) => {
      ...
      if (messageId.startsWith("temp-")) {
        toast.error("Attendez la fin de l'envoi du message");
        return;
      }
      ...
      try {
        ...
        const reason = err instanceof Error ? err.message : "network error";
        toast.error(`Échec de la réaction : ${reason}`);
    ...
    return (
      <div className={cn("flex flex-1 flex-col items-center justify-center", DOODLE_BG_CLASSES)}>
        ...
        <p className="mt-1 text-xs text-slate-600">
          Choisissez une conversation à gauche pour commencer à échanger
        </p>
      </div>
    );
  }
  ...
  const assignLabel = assignedAgentId
    ? (currentAssignee?.full_name ?? "Attribuée")
    : "Attribuer";
        ...
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center py-12">
            ...
            <p className="text-xs text-slate-600">
              Envoyez un modèle pour démarrer la conversation
            </p>
          </div>
        ) : (
