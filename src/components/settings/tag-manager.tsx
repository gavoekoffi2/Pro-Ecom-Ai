...
export function TagManager() {
  ...
  async function fetchTags(userId: string) {
    try {
      ...
      toast.error('Échec du chargement des étiquettes');
    } finally {
      setLoading(false);
    }
  }
  ...
  async function handleCreate() {
    ...
    try {
      ...
      toast.error('Échec de la création de l\'étiquette');
    } finally {
      setSaving(false);
    }
  }
  ...
  async function handleDelete() {
    ...
    try {
      ...
      toast.error('Échec de la suppression de l\'étiquette');
    } finally {
      setDeleting(false);
    }
  }
  ...
  return (
    <div className="space-y-4 mt-4">
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-lg font-semibold text-white">Étiquettes</h2>
          <p className="text-sm text-slate-400">Organisez vos contacts avec des étiquettes colorées.</p>
        </div>
        <Button
          onClick={() => {
            setNewTagName('');
      ...
      {tags.length === 0 ? (
        <Card className="bg-slate-900 border-slate-700 ring-0 ring-transparent">
          <CardContent className="flex flex-col items-center justify-center py-12 text-center">
            <p className="text-slate-400 text-sm">Aucune étiquette.</p>
            <p className="text-slate-500 text-xs mt-1">Créez des étiquettes pour catégoriser vos contacts.</p>
          </CardContent>
        </Card>
      ) : (
        <Card className="bg-slate-900 border-slate-700 ring-0 ring-transparent">
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent className="bg-slate-900 border-slate-700 sm:max-w-sm">
          <DialogHeader>
            <DialogTitle className="text-white">Nouvelle étiquette</DialogTitle>
            <DialogDescription className="text-slate-400">
              Créez une nouvelle étiquette avec un nom et une couleur.
            </DialogDescription>
          </DialogHeader>

          <div className="space-y-4 py-2">
            <div className="space-y-2">
              <Label className="text-slate-300">Nom de l&apos;étiquette</Label>
              <Input
            ...
            {/* Preview */}
            <div className="space-y-2">
              <Label className="text-slate-300">Aperçu</Label>
              <div>
                <span
      ...
      <Dialog open={deleteDialogOpen} onOpenChange={setDeleteDialogOpen}>
        <DialogContent className="bg-slate-900 border-slate-700 sm:max-w-sm">
          <DialogHeader>
            <DialogTitle className="text-white">Supprimer l&apos;étiquette</DialogTitle>
            <DialogDescription className="text-slate-400">
              Voulez-vous vraiment supprimer l&apos;étiquette « {tagToDelete?.name} » ? Elle sera
              retirée de tous les contacts. Cette action est irréversible.
            </DialogDescription>
          </DialogHeader>
          <DialogFooter className="bg-slate-900 border-slate-700">
            <Button
