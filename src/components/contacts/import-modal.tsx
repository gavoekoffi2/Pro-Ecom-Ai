...
export function ImportModal({ open, onOpenChange, onImported }: ImportModalProps) {
  ...
  async function handleFileChange(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    if (rows.length === 0) {
      toast.error('Aucune ligne valide. Vérifiez que le CSV a une colonne « phone ».');
      setParsedRows([]);
      return;
    }
    ...
    try {
      ...
      for (let i = 0; i < parsedRows.length; i += chunkSize) {
        ...
        const rows = chunk.map((row) => ({
          user_id: user.id,
          // Complete locally-typed numbers (e.g. Togolese 8-digit) to full
          // international format so WhatsApp can reach them.
          phone: toInternationalDigits(row.phone),
          name: row.name || null,
      ...
      if (failed > 0) {
        toast.error(`Échec de l'import de ${failed} contact${failed !== 1 ? 's' : ''}`);
      }
    } catch (err: unknown) {
      ...
      <DialogContent className="bg-slate-900 border-slate-700 text-slate-200 sm:max-w-lg">
        <DialogHeader>
          ...
          <DialogDescription className="text-slate-400">
            Importez un fichier CSV avec une colonne « phone » (obligatoire).
            Colonnes optionnelles : name, email, company.
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="space-y-4">
            ...
            {file ? (
              ...
              <>
                ...
                <p className="text-sm text-slate-400">
                  Cliquez pour importer un fichier CSV
                </p>
                <p className="text-xs text-slate-500">
                  CSV avec une colonne « phone » obligatoire
                </p>
              </>
            )}
          </div>
          ...
          {preview.length > 0 && !result && (
            <div className="space-y-2">
                ...
                <table className="w-full text-xs">
                  <thead>
                    <tr className="bg-slate-800">
                      <th className="px-3 py-1.5 text-left text-slate-400 font-medium">Téléphone</th>
              ...
              {parsedRows.length > 5 && (
                <p className="text-xs text-slate-500">
                  …et {parsedRows.length - 5} autres lignes
                </p>
              )}
            </div>
          )}
