...
export function PipelineSettings({
  ...
  async function handleAddStage() {
    ...
    const { data, error } = await supabase
      ...
      .single();
    if (error || !data) {
      toast.error("Échec de l'ajout de l'étape");
      return;
    }
  ...
  async function handleRemoveStage(stageId: string) {
    ...
    const { count } = await supabase
      ...
      .eq("stage_id", stageId);
    if (count && count > 0) {
      toast.error("Déplacez ou supprimez d'abord les affaires de cette étape");
      return;
    }
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-md bg-slate-900 border-slate-700 max-h-[85vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle className="text-white">Gérer le pipeline</DialogTitle>
        </DialogHeader>
        ...
        {showDeleteConfirm ? (
            ...
            <div className="flex items-center gap-3 rounded-lg border border-red-500/30 bg-red-500/10 p-4">
              ...
              <div>
                ...
                <p className="mt-1 text-xs text-slate-400">
                  Toutes les affaires de ce pipeline seront archivées. Cette
          ...
          <>
            <div className="grid gap-4 py-2">
              ...
              <div className="grid gap-2">
                <Label className="text-slate-300">Étapes</Label>
                <DndContext
                  sensors={sensors}
