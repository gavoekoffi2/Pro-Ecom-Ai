...
const SPEC_DEFAULT_STAGES = [
  ...
  { name: "Qualifié", color: "#eab308", position: 1 }, // yellow
  { name: "Proposition envoyée", color: "#f97316", position: 2 }, // orange
  { name: "Négociation", color: "#8b5cf6", position: 3 }, // purple
  { name: "Gagné", color: "#22c55e", position: 4 }, // green
];
...
export default function PipelinesPage() {
  ...
  const seedDefaultPipeline = useCallback(async (): Promise<Pipeline | null> => {
    ...
    const { data: pipeline, error } = await supabase
      .from("pipelines")
      .insert({ user_id: user.id, name: "Pipeline commercial" })
      .select()
      .single();
  ...
  const handleDealMoved = useCallback(
    async (dealId: string, newStageId: string) => {
      ...
      const { error } = await supabase
        ...
        .eq("id", dealId);
      if (error) {
        toast.error("Échec du déplacement de l'affaire");
        refreshDeals();
      }
    },
  ...
  async function handleCreatePipeline() {
    ...
    if (error || !pipeline) {
      toast.error("Échec de la création du pipeline");
      setCreating(false);
      return;
    }
          ...
          <DropdownMenu>
            ...
            >
              ...
              <span className="font-semibold">
                {selectedPipeline?.name ?? "Choisir un pipeline"}
              </span>
        ...
        <DialogContent className="sm:max-w-sm bg-slate-900 border-slate-700">
          ...
          <div className="py-2">
            ...
            <Input
              ...
              placeholder="Ex. : Ventes grands comptes"
            ...
            <p className="mt-2 text-xs text-slate-400">
              Les étapes par défaut (Nouveau prospect → Gagné) seront créées automatiquement.
            </p>
          </div>
          <DialogFooter className="bg-slate-900/50 border-slate-700">
            ...
            <Button
              ...
              className="bg-primary text-primary-foreground hover:bg-primary/90"
            >
              {creating ? "Création…" : "Créer le pipeline"}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
